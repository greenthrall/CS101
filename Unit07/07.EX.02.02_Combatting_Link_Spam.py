...
def compute_ranks(graph, k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            for node in graph:
                if page in graph[node] and not is_recipricol_link(graph, node, page, k):
...
def is_recipricol_link(graph, from_page, to_page, k):
    if k == 0:
        if from_page == to_page:
            ...
            return False
    if k == 1:
        if from_page in graph[to_page]:
            return True
        else:
            return False
    else:
        for v_page in graph[to_page]:
            if is_recipricol_link(graph, from_page, v_page, k - 1):
                return True
    return False


def new_compute_ranks(graph, k):
    d = 0.8 # damping factor
    numloops = 10
    ranks = {}
    npages = len(graph)
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not new_is_recipricol_link(graph, node, page, k):
                        newrank += d * (ranks[node] / len(graph[node]))
            newranks[page] = newrank
        ranks = newranks
    return ranks
...
def new_is_recipricol_link(graph, source, destination, k):
    if k == 0:
    ...
    for node in graph[destination]:
        if new_is_recipricol_link(graph, source, node, k - 1):
            return True
    return False
# For example
...
print new_compute_ranks(g, 0) # the a->a link is reciprocal

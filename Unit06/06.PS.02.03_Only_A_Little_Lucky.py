...
def ordered_search(index, ranks, keyword):
    ...
    while i < len(pages):
        best_page = pages[i]
        #print best_page
        if best_page not in v_retval:
            for candidate in pages:
                if candidate not in v_retval:
                    if ranks[candidate] > ranks[best_page]:
                        best_page = candidate
            v_retval.append(best_page)
        i += 1
    for candidate in pages:
        if candidate not in v_retval:
            v_retval.append(candidate)
    return v_retval


def new_ordered_search(index, ranks, keyword):
    pages = lookup(index, keyword)
...
print new_ordered_search(index, ranks, 'Hummus')
...
print new_ordered_search(index, ranks, 'the')
#>>> ['http://udacity.com/cs101x/urank/nickel.html',
#    'http://udacity.com/cs101x/urank/arsenic.html',
#    'http://udacity.com/cs101x/urank/hummus.html',
#    'http://udacity.com/cs101x/urank/index.html']
...
print new_ordered_search(index, ranks, 'babaganoush')
#>>> None
